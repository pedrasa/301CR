...
#include <vector>
using namespace std;
//rigidbody used for collision, but it also gives the affect of gravity to the plane, this is what makes physics affect the object
RigidBody::RigidBody()
{
	friction = 1;
	gravity = 0;
...
void RigidBody::SetVel(Vector3 _v)
{
	vel = _v;
}
...
{
	//this is using the bouding rect to check is two bodies are colliding, body a and body b
	Rect rcA = rbA.boundingRect;
	Rect rcB = rbB.boundingRect;
	//example aUR - a upper right and soon on, these are the vertices
	...
	//since we need to do this math for all 4 points in each boxe since we dont know which ones we exatcly know to check
	//and we use the axis vector to go trough all of em
